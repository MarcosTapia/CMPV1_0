...
class Usuarioturno_controller extends CI_Controller {
...
//        $this->gatewayRest =  $cadena;
    ...
    function verificaUsuarioOperador($usuario,$clave) {
        ...
        $data_string = json_encode($data);
        $ch = curl_init(RUTAWS.'usuarios_turnos/verifica_usuario_operador.php');
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
        curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        ...
        if ($datos->{'estado'}==1) {
            ...
            $user = $datos->{'usuario'}->{'usuario'};
            $pass = $datos->{'usuario'}->{'clave'};
            ...
            $apellido_materno = $datos->{'usuario'}->{'apellido_materno'};
            $this->session->set_userdata('nombre', $nombre." ".$apellido_paterno." ".$apellido_materno);
            $this->session->set_userdata('permisos', $datos->{'usuario'}->{'permisos'});
    ...
    function mostrarUsuariosTurnos() {
        if ($this->is_logged_in()){
            $dt = new DateTime("now", new DateTimeZone('America/Mexico_City'));
            $fechaIngreso = $dt->format("Y-m-d H:i:s"); 
            ...
            if ($datos->{'estado'}==1) {
                $data = array('usuarios'=>$datos->{'usuarios'},
                    ...
                    'nombre_Empresa'=>$this->nombreEmpresaGlobal,
                    'permisos' => $this->session->userdata('permisos'),
                    'opcionClickeada' => 'usuarios'
                        );
                $this->load->view('layouts/headerOperador_view',$data);
                $this->load->view('usuarios_turnos/adminUsersTurnos_view',$data);
                $this->load->view('layouts/pie_view',$data);
            } else {
                $this->load->view('layouts/headerAdministrador_view',$data);
                $this->load->view('principal_view',$data);
    ...
    function actualizarUsuarioTurno($idUsuarioOperador) {
        if ($this->is_logged_in()) {
            $dt = new DateTime("now", new DateTimeZone('America/Mexico_City'));
            $fechaIngreso = $dt->format("Y-m-d H:i:s"); 
            ...
            if ($datos->{'estado'}==1) {
                ...
                $this->load->view('layouts/headerOperador_view',$data);
                $this->load->view('usuarios_turnos/actualizaUsuarioTurno_view',$data);
                $this->load->view('layouts/pie_view',$data);
            } else {
                echo "error";
            }
    ...
    function actualizarUsuarioFromFormulario() {
        if ($this->is_logged_in()){
            if ($this->input->post('submit')){
                ...
                $idUsuarioOperador = $this->input->post("idUsuarioOperador");
                $usuario = $this->input->post("usuario");
                $clave = $this->input->post("clave");
                ...
                $data = array("idUsuarioOperador" => $idUsuarioOperador, 
                    "usuario" => $usuario, 
                    "clave" => $clave, 
                    "permisos" => $permisos, 
                    "nombre" => $nombre, 
                ...
                $data_string = json_encode($data);
                $ch = curl_init(RUTAWS.'usuarios_turnos/actualizar_usuarioturno.php');
                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
                curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                ...
                redirect('/index.php/usuarioturno_controller/mostrarUsuariosTurnos');
            }
        } else {
            redirect($this->cerrarSesion());
        }
    ...
    function eliminarUsuarioTurno($idUsuarioOperador) {
        if ($this->is_logged_in()){
            $data = array("idUsuarioOperador" => $idUsuarioOperador);
            $data_string = json_encode($data);
            $ch = curl_init(RUTAWS.'usuarios_turnos/borrar_usuarioturno.php');
            curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
            curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    ...
    function nuevoUsuarioTurno() {
        if ($this->is_logged_in()){
            ...
            $this->load->view('layouts/headerOperador_view',$data);
            $this->load->view('usuarios_turnos/nuevoUsuarioTurno_view',$data);
            $this->load->view('layouts/pie_view',$data);
        } else {
            redirect($this->cerrarSesion());
        }
    ...
    function nuevoUsuarioFromFormulario() {
        if ($this->is_logged_in()){
            if ($this->input->post('submit')){
                ...
                $data_string = json_encode($data);
                $ch = curl_init(RUTAWS.'usuarios_turnos/insertar_usuarioturno.php');
                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
                curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    ...
    function mostrarAtencion() {
        if ($this->is_logged_in()){
            ...
            $data = array(
                    'user'=>$this->session->userdata('user'),
                    'pass'=>$this->session->userdata('pass'),                    'idUsuarioOperador'=>$this->session->userdata('idUsuarioOperador'),
                    'permisos'=>$this->session->userdata('permisos'),
                    'usuarioDatos' => $this->session->userdata('nombre'),
                    'fecha' => $fechaIngreso,
                    'nombre_Empresa'=>$this->nombreEmpresaGlobal,
