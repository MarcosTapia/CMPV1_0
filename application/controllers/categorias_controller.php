...
//        $this->gatewayRest =  $cadena;
    ...
    function mostrarCategorias() {
        if ($this->is_logged_in()){
            ...
            $fechaIngreso = $dt->format("Y-m-d H:i:s"); 
            if ($datos->{'estado'}==1) {
                $data = array('categorias'=>$datos->{'categorias'},
                    'usuarioDatos' => $this->session->userdata('nombre'),
                    'fecha' => $fechaIngreso,
                    'nombre_Empresa'=>$this->nombreEmpresaGlobal,
                    'permisos' => $this->session->userdata('permisos'),
                ...
                $this->load->view('layouts/headerAdministrador_view',$data);
                $this->load->view('categorias/adminCategorias_view',$data);
                $this->load->view('layouts/pie_view',$data);
            } else {
                $this->load->view('layouts/headerAdministrador_view',$data);
                $this->load->view('principal_view',$data);
    ...
    function nuevoCategoria() {
        if ($this->is_logged_in()){
            ...
            $this->load->view('layouts/headerAdministrador_view',$data);
            $this->load->view('categorias/nuevoCategoria_view',$data);
            $this->load->view('layouts/pie_view',$data);
        } else {
            redirect($this->cerrarSesion());
        }
    ...
    function nuevoCategoriaFromFormulario() {
        if ($this->is_logged_in()){
            if ($this->input->post('submit')){
                ...
                redirect('/index.php/categorias_controller/mostrarCategorias'); 
            }
        } else {
            redirect($this->cerrarSesion());
        }
    ...
    function actualizarCategoria($idCategoria) {
        if ($this->is_logged_in()){
            ...
            if ($datos->{'estado'}==1) {
                ...
                $this->load->view('layouts/headerAdministrador_view',$data);
                $this->load->view('categorias/actualizaCategoria_view',$data);
                $this->load->view('layouts/pie_view',$data);
            } else {
                echo "error";
            }
