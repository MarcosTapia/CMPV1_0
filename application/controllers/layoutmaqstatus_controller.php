...
class Layoutmaqstatus_controller extends CI_Controller {
    ...
    function mostrarlugares() {
        if ($this->is_logged_in()){
            $url = RUTAWS.'layout_maq_status/obtener_layout_maq_status.php';
            $ch = curl_init($url);
            ...
            $fechaIngreso = $dt->format("Y-m-d H:i:s"); 
            if ($datos->{'estado'}==1) {
                $data = array('layoutmaqstatus'=>$datos->{'layoutmaqstatus'},
                    'usuarioDatos' => $this->session->userdata('nombre'),
                    'fecha' => $fechaIngreso,
                    'nombre_Empresa'=>$this->nombreEmpresaGlobal,
                    'permisos' => $this->session->userdata('permisos'),
                ...
                $this->load->view('layouts/headerAdministrador_view',$data);
                $this->load->view('layout_maq_status/adminLayoutmaqstatus_view',$data);
                $this->load->view('layouts/pie_view',$data);
            } else {
                $this->load->view('layouts/headerAdministrador_view',$data);
                $this->load->view('principal_view',$data);
    ...
    function nuevoLugar() {
        if ($this->is_logged_in()){
            ...
            $this->load->view('layouts/headerAdministrador_view',$data);
            $this->load->view('layout_maq_status/nuevoLayoutMaqStatus_view',$data);
            $this->load->view('layouts/pie_view',$data);
        } else {
            redirect($this->cerrarSesion());
        }
    ...
    function nuevoLugarFromFormulario() {
        if ($this->is_logged_in()){
            if ($this->input->post('submit')){
                ...
                $ch = curl_init(RUTAWS.'layout_maq_status/insertar_lugar.php');
                ...
                redirect('/index.php/layoutmaqstatus_controller/mostrarlugares'); 
            }
        } else {
            redirect($this->cerrarSesion());
        }
    ...
    function actualizarMaqLugar($id) {
        if ($this->is_logged_in()){
            $dt = new DateTime("now", new DateTimeZone('America/Mexico_City'));
            $fechaIngreso = $dt->format("Y-m-d H:i:s"); 
            $url = RUTAWS.'layout_maq_status/obtener_lugar_por_id.php?id='.$id;
            $ch = curl_init($url);
            curl_setopt($ch, CURLOPT_TIMEOUT, 5);
            curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            ...
            if ($datos->{'estado'}==1) {
                ...
                $this->load->view('layouts/headerAdministrador_view',$data);
                $this->load->view('layout_maq_status/actualizaLayoutMaqStatus_view',$data);
                $this->load->view('layouts/pie_view',$data);
            } else {
                echo "error";
            }
    ...
    function actualizarLugarFromFormulario() {
        if ($this->is_logged_in()){
            if ($this->input->post('submit')){
                ...
                $data_string = json_encode($data);
                $ch = curl_init(RUTAWS.'layout_maq_status/actualizar_lugar.php');
                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
                curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    ...
    function eliminarMaqLugar($id) {
        if ($this->is_logged_in()){
            $data = array("id" => $id);
            $data_string = json_encode($data);
            $ch = curl_init(RUTAWS.'layout_maq_status/borrar_lugar.php');
            curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
            curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
